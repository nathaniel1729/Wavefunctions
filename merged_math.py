...
def Psi_harmonic(x,t,m,n,a_n,phase=0):
    ...
    return psi_harmonic(x,m,n,a_n)*np.exp(-1j*w*t+phase*1j)
x=np.linspace(-3,3)
a=Hermites(48)
# for i in range(48):
...
def draw_frames(f,X,Y,t,width=4,show_prob=False):
    """
...
def Psi_hydrogen_spherical(X,t,w,n,l,m,phase=0):
    """
    ...
    """
    #X=r,theta,phi
    E_1=-(w/(2*h_**2))*(e**2/(4*np.pi*ep_0))**2
    E=E_1/n**2#(.511e6/13.606)*E_1-
    L=Laguerre(n-l-1,2*l+1)
    return R(X[0],w,n,l,L)*Y(X[1],X[2],l,m)*time_cycle(t,E,phase) 
def Psi_hydrogen_cartesian(X,t,w,n,l,m,phase=0):
    """
...
def Packet_hydrogen(X,t,w,n_mean,n_s,l_mean,l_s,m_mean,m_s):
...
def Packet_hydrogen2(X,t,w,n_mean,n_s,l_mean,l_s,m_mean,m_s,initial):
    x,y,z=X
...
def Psi_triangle(X,t,m,q,r):
    ...
    return sum(np.exp(1j*pre_coeff_i)*Psi_2d([X2,Y2],t,m,V) for g,pre_coeff_i,V in zip(gs,pre_coeffs,Vs))*in_triangle(X[0],X[1])



def Psi_triangle_2(X,t,m,q,r):
    return Psi_triangle(X,t,m,q*3,r+1)
...
help_function=lambda data,param: Psi_triangle_2([data[0],data[1]],data[2],data[3],param[0],param[1])
...
def set_function_Magnitude():
    #return lambda view,params: Packet_harmonic(view[0],view[1],params[0],params[1],params[2])

    # return lambda view, params:Psi_m_circular(
    #         view,params[0][0],params[0][1],params[0][2],params[0][3],params[0][4]
    #     )+Psi_m_circular(
    #         view,params[1][0],params[1][1],params[1][2],params[1][3],params[1][4]
    #     )#params=[t,w,J,n,N]
    
    

    # #attach help to P2
    # P=Packet_attatched(help_function,P2_data,data_init,parameters)
    # return lambda view,params: P([view[0],view[1],params[0],w])#params=[t]

    return lambda view,params: Psi_triangle_2(view,params[0],params[1],params[2],params[3])#params=[t,m,q,r]
